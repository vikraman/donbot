...
const wikiSearchResult = (title: string) => ({
  json: async () => ({ query: { search: [{ title }] } })
}) as unknown as Response

describe('Ask testing Hubot scripts', () => {
  const state = setupRobot('Ask.ts', { envVars: ['GEMINI_API_KEY'] })
  ...
  it('should send gemini the raw, unstripped question', async () => {
    const { robot } = state
    process.env.GEMINI_API_KEY = 'test-key'
    let requestBody: GeminiRequest | null = null
    mock.method(global, 'fetch', async (url: string, options: { body: string }) => {
