...
const fakeChannel = messagesById => ({
  messages: {
    fetch: async ({ message }) => {
      if (!messagesById[message]) throw new Error('not found')
...
describe('History testing Hubot scripts', () => {
  const state = setupRobot('History.mjs', { deferLoad: true })
  ...
  it('should log a user-message entry for every message with a raw discord id', async () => {
    ...
    const entries = robot.brain.get('history:test-room')
  ...
  it('should log negative valence when a positive reaction is removed', async () => {
    ...
    robot.adapter.client.emit('messageReactionRemove', reaction, { bot: false, username: 'bob' })
    await new Promise(resolve => setImmediate(resolve))
  ...
  it('should ignore reactions from bots and reactions with no valence mapping', async () => {
    ...
    robot.adapter.client.emit('messageReactionAdd', {
