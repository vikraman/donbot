...
const pollKey = room => `poll:${room}`

const countVotes = async (message, emoji) => {
...
export default async (robot) => {
  robot.respond(/poll results$/i, async res => {
    const poll = robot.brain.get(pollKey(res.message.room))
    if (!poll) {
      await res.send('No poll has been run in this room yet.')
      return
    }

    const rawMessage = res.message.user && res.message.user.message
