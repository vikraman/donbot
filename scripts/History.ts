...
import type { Robot, DiscordChannel, DiscordReaction, DiscordUser } from 'hubot'

import { rawMessageOf } from './lib/discordMessage.ts'
import { required } from './lib/match.ts'
import { mentionFor } from './lib/mention.ts'

type EntryKind = 'bot-message' | 'user-message' | 'reaction-feedback'
...
const REACTION_VALENCE: Record<string, number> = {
...
// "last X" spans the prior period, not a longer lookback
const LAST_PERIOD_OFFSET: Record<string, number> = {
  'last week': 7 * DAY_MS,
  'last month': 30 * DAY_MS,
  'last year': 365 * DAY_MS
}

const appendEntry = (robot: Robot, room: string, entry: HistoryEntry) => {
  const key = HISTORY_KEY(room)
  const entries = robot.brain.get<HistoryEntry[]>(key) || []
  entries.push(entry)
  robot.brain.set(key, entries)
}

const logBotMessage = (robot: Robot, room: string, messageId: string) => {
  appendEntry(robot, room, { id: messageId, ts: Date.now(), kind: 'bot-message' })
}

const netScores = (entries: HistoryEntry[]): Map<string, number> => {
  const scores = new Map<string, number>()
  for (const entry of entries) {
    if (entry.kind !== 'reaction-feedback') continue
    scores.set(entry.id, (scores.get(entry.id) || 0) + (entry.valence || 0))
  }
  return scores
}
...
const messageCounts = (entries: HistoryEntry[], { since, until }: WindowRange = {}): Map<string, number> => {
  const counts = new Map<string, number>()
  for (const entry of entries) {
    if (entry.kind !== 'user-message' || !entry.authorId) continue
...
const fetchText = async (channel: DiscordChannel, messageId: string): Promise<string | null> => {
  try {
...
const authorOf = (robot: Robot, entries: HistoryEntry[], messageId: string): string | null => {
  const entry = entries.find(e => e.id === messageId && e.kind === 'user-message')
  return entry ? entry.authorId || null : null
}

const describeMessage = async (robot: Robot, channel: DiscordChannel, entries: HistoryEntry[], messageId: string, score: number): Promise<string> => {
  const text = await fetchText(channel, messageId)
  const authorId = authorOf(robot, entries, messageId)
  const mention = mentionFor({ id: authorId ?? undefined })
  if (text === null) {
...
const topEntry = (scores: Map<string, number>, { sign = 1, since, until, entries = [] }: TopEntryOptions = {}): [string, number] | undefined => {
  const filtered = [...scores.entries()].filter(([id, score]) => {
    ...
    if (since != null || until != null) {
      ...
      const inWindow = (e: HistoryEntry) => (since == null || e.ts >= since) && (until == null || e.ts < until)
...
// period name -> { since, until }; until only set for "last X"
const resolveWindow = (period?: string): WindowRange => {
  if (!period) return {}
  ...
  // unknown period: open window, not a NaN cutoff
  if (span == null) return {}
  return { since: Date.now() - span }
}
...
export default async (robot: Robot) => {
  ...
  if (client && typeof client.on === 'function') {
    const recordFeedback = (removed: boolean) => async (reaction: DiscordReaction, user: DiscordUser) => {
      if (user.bot) return
  ...
  robot.respond(/top message(?: (today|day|this week|week|last week|this month|month|last month|this year|year|last year))?$/i, async res => {
    ...
    const entries = robot.brain.get<HistoryEntry[]>(HISTORY_KEY(res.message.room)) || []
  ...
  robot.respond(/last message from @?([\w .-]+?)\??\s*$/i, async res => {
    ...
    const entries = robot.brain.get<HistoryEntry[]>(HISTORY_KEY(res.message.room)) || []
    const sole = users[0]
    const userMessages = sole
      ? entries.filter(e => e.kind === 'user-message' && e.authorId === sole.id).sort((a, b) => b.ts - a.ts)
      : []
  ...
  robot.respond(/(most|least) active user(?: (today|day|this week|week|last week|this month|month|last month|this year|year|last year))?$/i, async res => {
    ...
    const most = required(res.match, 1).toLowerCase() === 'most'
