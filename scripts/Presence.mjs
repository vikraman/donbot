...
// Short verbatim lines from Donbot, head of the Robot Mafia.
const DONBOT_QUOTES = [
...
const buildStatuses = robot => [
  ...
  () => {
    const last = robot.brain.get(LAST_MESSAGE_KEY)
    if (!last) return null
...
const updatePresence = robot => {
  const client = robot.adapter && robot.adapter.client
  if (!client || !client.isReady || !client.isReady() || !client.user) return

  const statuses = buildStatuses(robot)
  let text = null
  while (text === null) {
    text = pick(statuses)()
  }
  client.user.setActivity(text, { type: 4 }) // 4 = Custom
}
